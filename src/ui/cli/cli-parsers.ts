...
export function parseScaffoldArgs(values: Record<string, string | boolean | undefined>): ProjectConfig {
  ...
  if (!(pm in PM_COMMANDS)) {
    throw new Error(`Invalid package manager: ${pm}. Must be one of: ${Object.keys(PM_COMMANDS).join(", ")}`);
  }
    ...
    if (fbModulesStr) {
      ...
      for (const m of specified) {
        if (!validFbModules.includes(m)) {
          throw new Error(`Invalid Firebase module: ${m}. Valid options: ${validFbModules.join(", ")}`);
        }
      }
      modules = specified;
    }
